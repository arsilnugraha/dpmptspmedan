...
class Berita extends CI_Controller {
   public function __construct()
   {
          ...
          $this->load->model('panel/module/berita_model','dbObject');
   }
   ...
   public function index() {
      ...
      $output['icon'] = 'fa fa-newspaper-o';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/berita/berita_view';
		$output['after_page'] = 'panel/module/berita/berita_after_page';
   ...
   public function ajax_list() {
		...
		foreach ($list as $r) {
			...
			$html = '<div class="btn-group btn-group-solid">
                  <a class="btn btn-'.($r->status == 'Y' ? 'warning' : 'success').' btn-xs" data-toggle="tooltip" title="'.($r->status == 'Y' ? 'Hentikan Publikasi Berita' : 'Publikasikan Berita').'" href="' . base_url('panel/module/berita/publish') . '/' . $r->id . '/' . $r->status . '">
                     <span class="'.($r->status == 'Y' ? 'fa fa-times-circle' : 'fa fa-check-circle').'"></span>
                  </a>
                  <a class="btn btn-primary btn-xs" data-toggle="tooltip" title="Ubah Data" href="' . base_url('panel/module/berita/edit') . '/' . $r->id . '">
                     <span class="fa fa-pencil"></span>
                  </a>
                  <a class="btn btn-danger btn-xs" data-toggle="tooltip" title="Hapus Data" href="' . base_url('panel/module/berita/hapus/') . $r->id . '" onclick="return confirm(\'Apa anda yakin untuk hapus Data ini?\')">
   ...
   public function tambah($param1='') {
      ...
      $output['icon'] = 'fa fa-cog';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/berita/berita_tambah';
		$output['after_page'] = 'panel/module/berita/berita_after_page';
      $this->load->view('panel/master_page', $output);
   }
   ...
   public function hapus() {
      $berita = $this->dbObject->get_by_id($this->uri->segment(5));
      if($berita->gambar) {
         $path = './assets/foto_berita/'; 
   ...
   public function edit($param1) {
      if ($param1=='do_edit') {
         ...
         $tgl_sekarang = date("Ymd");
         if($this->upload->do_upload('fupload')) {
            $berita = $this->dbObject->get_by_id($this->input->post('id'));
            if($berita->gambar) {
               $path = './assets/foto_berita/';
               @unlink($path.$berita->gambar);
            }
      ...
      $output['judul_besar'] = 'Berita';
		...
		$output['menu_aktif'] = 'm_berita';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/berita/berita_edit';
		$output['after_page'] = 'panel/module/berita/berita_after_page';
      $id = $this->uri->segment(5);
      $output['berita'] = $this->dbObject->get_by_id($id);
      $this->load->view('panel/master_page', $output);
   }
