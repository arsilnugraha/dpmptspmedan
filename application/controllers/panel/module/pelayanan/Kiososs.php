...
class Kiososs extends CI_Controller {
   public function __construct()
   {
          ...
          $this->load->model('panel/module/pelayanan/kiososs_model','dbObject');
   }

   public function index() {
      $output['judul_besar'] = 'Pelayanan';
		$output['judul_kecil'] = 'Kios OSS';
		$output['menu_aktif'] = 'm_kiososs';
      $output['icon'] = 'fa fa-clipboard';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/pelayanan/kiososs/kiososs_view';
		$output['after_page'] = 'panel/module/pelayanan/kiososs/kiososs_after_page';
   ...
   public function ajax_list() {
		...
		foreach ($list as $r) {
			...
			$html = '<div class="btn-group btn-group-solid">
                  <a class="btn btn-'.($r->status == 'Y' ? 'warning' : 'success').' btn-xs" data-toggle="tooltip" title="'.($r->status == 'Y' ? 'Hentikan Publikasi Berita' : 'Publikasikan Berita').'" href="' . base_url('panel/module/pelayanan/kiososs/publish') . '/' . $r->id . '/' . $r->status . '">
                     <span class="'.($r->status == 'Y' ? 'fa fa-times-circle' : 'fa fa-check-circle').'"></span>
                  </a>
                  <a class="btn btn-primary btn-xs" data-toggle="tooltip" title="Ubah Data" href="' . base_url('panel/module/pelayanan/kiososs/edit') . '/' . $r->id . '">
                     <span class="fa fa-pencil"></span>
                  </a>
                  <a class="btn btn-danger btn-xs" data-toggle="tooltip" title="Hapus Data" href="' . base_url('panel/module/pelayanan/kiososs/hapus/') . $r->id . '" onclick="return confirm(\'Apa anda yakin untuk hapus Data ini?\')">
   ...
   public function tambah($param1='') {
      ...
      if ($param1=='do_create') {
         $config['upload_path']          = 'assets/foto_beritakiososs';
         ...
         redirect('panel/module/pelayanan/kiososs');
      }
      ...
      $output['icon'] = 'fa fa-cog';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/pelayanan/kiososs/kiososs_tambah';
		$output['after_page'] = 'panel/module/pelayanan/kiososs/kiososs_after_page';
      $this->load->view('panel/master_page', $output);
   }
   ...
   public function hapus() {
      $kiososs = $this->dbObject->get_by_id($this->uri->segment(6));
      if($kiososs->gambar) {
         $path = './assets/foto_beritakiososs/'; 
         @unlink($path.$kiososs->gambar);
      }
   ...
   public function edit($param1) {
      if ($param1=='do_edit') {
         ...
         $tgl_sekarang = date("Ymd");
         if($this->upload->do_upload('fupload')) {
            $kiososs = $this->dbObject->get_by_id($this->input->post('id'));
            if($kiososs->gambar) {
               $path = './assets/foto_beritakiososs/';
               @unlink($path.$kiososs->gambar);
            }
      ...
      $output['judul_besar'] = 'Kamis Ceria';
		...
		$output['menu_aktif'] = 'm_kiososs';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/pelayanan/kiososs/kiososs_edit';
		$output['after_page'] = 'panel/module/pelayanan/kiososs/kiososs_after_page';
      $id = $this->uri->segment(6);
      $output['kiososs'] = $this->dbObject->get_by_id($id);
      $this->load->view('panel/master_page', $output);
   }
