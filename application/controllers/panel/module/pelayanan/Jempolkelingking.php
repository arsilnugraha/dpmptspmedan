...
class Jempolkelingking extends CI_Controller {
   public function __construct()
   {
          parent::__construct();
          if ($this->session->userdata('login') == 0 || $this->session->userdata('leveluser') != ('admin' || 'operator')) redirect('panel/auth/logout');
          $this->load->model('panel/module/pelayanan/jempolkelingking_model','dbObject');
   }

   public function index() {
      $output['judul_besar'] = 'Pelayanan';
		$output['judul_kecil'] = 'Jempol Kelingking';
		$output['menu_aktif'] = 'm_jempolkelingking';
      $output['icon'] = 'fa fa-clipboard';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/pelayanan/jempolkelingking/jempolkelingking_view';
		$output['after_page'] = 'panel/module/pelayanan/jempolkelingking/jempolkelingking_after_page';
   ...
   public function ajax_list() {
		...
		foreach ($list as $r) {
			...
			$html = '<div class="btn-group btn-group-solid">
                  <a class="btn btn-'.($r->status == 'Y' ? 'warning' : 'success').' btn-xs" data-toggle="tooltip" title="'.($r->status == 'Y' ? 'Hentikan Publikasi Berita' : 'Publikasikan Berita').'" href="' . base_url('panel/module/pelayanan/jempolkelingking/publish') . '/' . $r->id . '/' . $r->status . '">
                     <span class="'.($r->status == 'Y' ? 'fa fa-times-circle' : 'fa fa-check-circle').'"></span>
                  </a>
                  <a class="btn btn-primary btn-xs" data-toggle="tooltip" title="Ubah Data" href="' . base_url('panel/module/pelayanan/jempolkelingking/edit') . '/' . $r->id . '">
                     <span class="fa fa-pencil"></span>
                  </a>
                  <a class="btn btn-danger btn-xs" data-toggle="tooltip" title="Hapus Data" href="' . base_url('panel/module/pelayanan/jempolkelingking/hapus/') . $r->id . '" onclick="return confirm(\'Apa anda yakin untuk hapus Data ini?\')">
   ...
   public function tambah($param1='') {
      ...
      if ($param1=='do_create') {
         $config['upload_path']          = 'assets/foto_beritajempolkelingking';
         ...
         redirect('panel/module/pelayanan/jempolkelingking');
      }
      ...
      $output['icon'] = 'fa fa-cog';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/pelayanan/jempolkelingking/jempolkelingking_tambah';
		$output['after_page'] = 'panel/module/pelayanan/jempolkelingking/jempolkelingking_after_page';
      $this->load->view('panel/master_page', $output);
   }
   ...
   public function hapus() {
      $jempolkelingking = $this->dbObject->get_by_id($this->uri->segment(6));
      if($jempolkelingking->gambar) {
         $path = './assets/foto_beritajempolkelingking/'; 
         @unlink($path.$jempolkelingking->gambar);
      }
   ...
   public function edit($param1) {
      if ($param1=='do_edit') {
         ...
         $tgl_sekarang = date("Ymd");
         if($this->upload->do_upload('fupload')) {
            $jempolkelingking = $this->dbObject->get_by_id($this->input->post('id'));
            if($jempolkelingking->gambar) {
               $path = './assets/foto_beritajempolkelingking/';
               @unlink($path.$jempolkelingking->gambar);
            }
      ...
      $output['judul_besar'] = 'Kamis Ceria';
		...
		$output['menu_aktif'] = 'm_jempolkelingking';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/pelayanan/jempolkelingking/jempolkelingking_edit';
		$output['after_page'] = 'panel/module/pelayanan/jempolkelingking/jempolkelingking_after_page';
      $id = $this->uri->segment(6);
      $output['jempolkelingking'] = $this->dbObject->get_by_id($id);
      $this->load->view('panel/master_page', $output);
   }
