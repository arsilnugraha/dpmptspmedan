...
class Tentang extends CI_Controller {
   public function __construct()
   {
          ...
          $this->load->model('panel/module/profil/tentang_model','dbObject');
   }

   public function index() {
      $output['judul_besar'] = 'Profil';
		$output['judul_kecil'] = 'Tentang';
		$output['menu_aktif'] = 'm_tentang';
      $output['icon'] = 'fa fa-institution';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/profil/tentang/tentang_view';
		$output['after_page'] = 'panel/module/profil/tentang/tentang_after_page';

      $output['tentang'] = $this->dbObject->get_by_id(1);
   ...
   public function edit($param1='') {
      if ($param1=='do_edit') {
         $datatentang = array(
            'tentang' => $this->input->post('isi_tentang'),
         );

         if($this->dbObject->update(['id' => $this->input->post('id')], $datatentang)==TRUE){
            $this->session->set_flashdata('notif','<div class="alert alert-success">Tentang berhasil diubah</div>');
         }else {
            $this->session->set_flashdata('notif','<div class="alert alert-danger">Tentang gagal diubah</div>');
         }
         redirect('panel/module/tentang');
      }
      $output['judul_besar'] = 'Profil';
		$output['judul_kecil'] = 'Tentang';
		$output['menu_aktif'] = 'm_tentang';
      $output['icon'] = 'fa fa-cog';
		$output['parent_master'] = 'active open';
		$output['content'] = 'panel/module/profil/tentang/tentang_edit';
		$output['after_page'] = 'panel/module/profil/tentang/tentang_after_page';
