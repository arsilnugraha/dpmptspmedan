<script type="text/javascript">
    ...
    TableAjax = function() {
        var handleRecords = function() {
            table = $('#datatable_ajax').DataTable({
                ...
                "order": [],
                "ajax": {
                    "url": "<?php echo site_url('panel/module/portal/ajax_list') ?>",
                    "type": "POST",
    ...
    $(document).ready(function() {
        ...
        $('#form').submit(function(e) {
            ...
            if (save_method == 'add') {
                url = "<?php echo site_url('panel/module/portal/ajax_add') ?>";
            } else {
                url = "<?php echo site_url('panel/module/portal/ajax_update') ?>";
            }
    ...
    function add_portal() {
        ...
        $('.modal-title').text('Tambah Portal');
    }
    ...
    function edit_portal(id) {
        ...
        $.ajax({
            url: "<?php echo site_url('panel/module/portal/ajax_edit/') ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                $('[name="id"]').val(data.id);
                $('[name="portal"]').val(data.nama_portal);
                $('[name="deskripsi"]').val(data.deskripsi);
                $('[name="link"]').val(data.link);
                var file_logo = data.logo;
                if(file_logo) {
                  var fileUrl  = '<?php echo base_url("/assets/logo_portal/") ?>' + file_logo;
    ...
    function delete_portal(id) {
        if (confirm('Are you sure delete this data?')) {
            $.ajax({
                url: "<?php echo site_url('panel/module/portal/ajax_delete/') ?>" + id,
                type: "GET",
                dataType: "JSON",
                data: function(dtRequest) {
                    dtRequest['<?php echo $this->security->get_csrf_token_name(); ?>'] = '<?php echo $this->security->get_csrf_hash(); ?>';
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
               <h3 class="modal-title">Form Portal</h3>
                ...
                <div class="form-body">
                    <div class="form-group">
                        <label class="col-sm-3 control-label">Nama Portal :</label>
                           <div class="col-sm-9">
                              <input name="portal" placeholder="Nama Portal" class="form-control" type="text">
