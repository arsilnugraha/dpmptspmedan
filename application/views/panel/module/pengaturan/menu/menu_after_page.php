<script type="text/javascript">
    ...
    TableAjax = function() {
        var handleRecords = function() {
            table = $('#datatable_ajax').DataTable({
                ...
                "order": [],
                "ajax": {
                    "url": "<?php echo site_url('panel/module/menu/ajax_list') ?>",
                    "type": "POST",
    ...
    $(document).ready(function() {
        ...
        $('#form').submit(function(e) {
            ...
            if (save_method == 'add') {
                url = "<?php echo site_url('panel/module/menu/ajax_add') ?>";
            } else {
                url = "<?php echo site_url('panel/module/menu/ajax_update') ?>";
            }
        ...
        $('#datatable_ajax').on('click', '#status', function() {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo site_url('panel/module/menu/ajax_updatestatus') ?>",
    ...
    function edit_menu(id) {
        ...
        $.ajax({
            url: "<?php echo site_url('panel/module/menu/ajax_edit/') ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                $('[name="id"]').val(data.id);
    ...
    function delete_menu(id) {
        if (confirm('Are you sure delete this data?')) {
            $.ajax({
                url: "<?php echo site_url('panel/module/menu/ajax_delete/') ?>" + id,
                type: "GET",
                dataType: "JSON",
                data: function(dtRequest) {
                    dtRequest['<?php echo $this->security->get_csrf_token_name(); ?>'] = '<?php echo $this->security->get_csrf_hash(); ?>';
